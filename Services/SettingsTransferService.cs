...
/// <summary>
/// 导入和导出 Classcaller 设置及 AppData 数据。
...
public sealed class SettingsTransferService
{
    ...
    public async Task ImportAsync(Stream source, CancellationToken cancellationToken = default)
    {
        ...
        var stagingRoot = Path.Combine(appDataParent, $".Classcaller-import-{operationId}");
        var stagingAppData = Path.Combine(stagingRoot, "AppData");
        var backupPath = Path.Combine(appDataParent, $".Classcaller-backup-{operationId}");
