...
/// 置顶增强引擎。
///
/// 目标：把 Classcaller 的悬浮窗 / 点名结果窗口的置顶优先级提升到用户态 Win32
/// 所能达到的最高级别 —— 置顶带（topmost band）的最顶端，并在任何其它窗口
/// 抢占前台后立即夺回。
///
/// 多机制同时生效（互相独立、可单独开关）：
///   1. 【高频 Z 序重推】以极短周期（默认 250ms，Classcaller 自带循环为 3000ms）
...
///      类型名 / 标题关键词识别 Classcaller 的窗口（跨插件程序集隔离，
///      无需编译期依赖）。
...
{
    /// <summary>已知的 Classcaller 窗口类型名（与程序集匹配互为兜底）。</summary>
    ...
    private readonly TopmostSetting _settings;
    ...
    public TopmostEnhancerService(ILogger<TopmostEnhancerService> logger, TopmostSetting settings)
    {
        _logger = logger;
        _settings = settings;
    ...
    public void Start()
    {
        ...
        // 立即执行一次扫描，让已打开的 Classcaller 窗口马上获得最高置顶。
        ScanAndApply();
        _logger.LogInformation("Classcaller 置顶增强引擎已启动（周期 {Interval}ms，前台钩子 {Hook}）。",
    ...
    {
        if (e.PropertyName == nameof(TopmostSetting.IntervalMs))
        {
            RestartTimer();
        }
        else if (e.PropertyName == nameof(TopmostSetting.EnableForegroundHook))
        {
            if (_settings.EnableForegroundHook)
            {
                InstallForegroundHook();
    ...
    private void ScanAndApply()
    {
        try
        {
            ...
            foreach (var hwnd in current)
            {
                ...
                {
                    _logger.LogInformation("发现 Classcaller 窗口，加入最高置顶守卫：HWND=0x{Hwnd}", hwnd.ToString("X"));
                }
            }
    ...
    /// <summary>判断某个 Avalonia 窗口是否属于 Classcaller。</summary>
    private bool IsClasscallerWindow(Window window)
    {
        // 1) 程序集名匹配（跨插件隔离场景最可靠）：Classcaller.Plugin2
        try
        {
            var assemblyName = window.GetType().Assembly.GetName().Name;
            if (assemblyName is not null &&
                assemblyName.StartsWith("Classcaller", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        }
