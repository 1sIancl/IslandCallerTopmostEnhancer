...
using Classcaller.ViewModels;
using Microsoft.Extensions.Logging;

namespace Classcaller.Views;

[SettingsPageInfo("plugins.Classcaller", "Classcaller 设置", "\uED39", "\uECF8", SettingsPageCategory.External)]
public partial class SettingPage : SettingsPageBase
{
    ...
    private async void ExportSettingsButton_OnClick(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        try
        {
            var filePath = await PlatformServices.FilePickerService.SaveFilePickerAsync(
                ...
                {
                    Title = "导出 Classcaller 数据包",
                    SuggestedFileName = $"Classcaller-{DateTime.Now:yyyyMMddHHmmss}.iscdoc",
                    DefaultExtension = ".iscdoc",
                    FileTypeChoices =
                    [
                        new FilePickerFileType("Classcaller 数据包")
                        {
                            Patterns = ["*.iscdoc"]
                        }
                    ]
            ...
            await CommonTaskDialogs.ShowDialog("导出完成", "Classcaller 设置和 AppData 已成功导出。", this);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "导出 Classcaller 数据包失败");
    ...
    private async void ImportSettingsButton_OnClick(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                Header = "导入 Classcaller 数据包",
                Content = "导入将清空现有的 Classcaller 设置、名单、历史记录以及 AppData 下的其他插件数据，并使用数据包内容完全替换。此操作不可逆，确定要继续吗？",
                XamlRoot = this,
                Buttons =
                {
            ...
            var temporaryPackagePath = Path.Combine(
                Path.GetTempPath(),
                $"Classcaller-import-{Guid.NewGuid():N}.iscdoc");
            try
            {
            ...
            finally
            {
                ...
                {
                    logger.LogWarning(cleanupException, "清理临时 Classcaller 数据包失败：{Path}", temporaryPackagePath);
                }
            }
        ...
        {
            logger.LogError(ex, "导入 Classcaller 数据包失败");
